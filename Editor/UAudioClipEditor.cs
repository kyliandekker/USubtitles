...
namespace UAudio.USubtitles.Editor
{
	class Interaction
	{
		public InteractionType LastInteraction = InteractionType.TimelineInteraction_None;
		public int Index = -1;
	}
	...
	public class UAudioClipEditor : UnityEditor.Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			if (_clip.Clip != null)
			{
				if (_currentInteraction.Index > _clip.Dialogue.Count)
					_currentInteraction.Index = -1;

				if (
					_currentInteraction.Index >= 0 && _currentInteraction.Index < _clip.Dialogue.Count &&
					(_currentDialogueItem == null || serializedObject.FindProperty("Dialogue").GetArrayElementAtIndex(_currentInteraction.Index) != _currentDialogueItem))
